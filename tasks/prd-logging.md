...
- Make the log format easy for a human to read (no color codes, no timestamps per line).
...
3. The system must write every log message (info, warn, success, error) printed to the terminal to the log file, in the format: `TYPE_OF_LOG message` (e.g., `info Homebrew installed successfully`).
4. The log file must not contain any color codes or terminal formatting.
5. The log file must be created at the start of the run, and logging must continue even if the script fails partway.
6. If the log file cannot be created, the script must fail immediately with an appropriate error message.
7. (Phase 1) Logging is required for the unattended setup script only. (If supporting the interactive script is trivial, it may be included.)

## Non-Goals (Out of Scope)

- No requirement to log timestamps for each message.
...
- The logging functions in `utils.sh` may need to be updated to support dual output (terminal and log file).
...
- The log file contains all terminal output messages, formatted as specified, with no color codes.
